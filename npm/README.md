...
`doctor` checks config, credentials, subscription entitlement, provider access, and backend reachability, then tells you or your agent exactly what to fix.

## Let your agent set up FlowLeap

If you would rather not configure a CLI yourself, paste the prompt below into your chosen agent. This works with local agent harnesses that can run shell commands and access your filesystem, such as Claude Code, Codex, Cursor, and Gemini CLI. A web-only chat cannot install or persist this local integration.
...
For manual setup, these are the persistent harness commands and files:

| Harness | MCP registration | Skills/rules |
|---------|------------------|--------------|
| Claude Code | `claude mcp add --transport stdio --scope user flowleap -- flowleap mcp` | `flowleap skills install` (user-wide) |
| Codex | `codex mcp add flowleap -- flowleap mcp` (user-wide) | `flowleap skills install --target codex` (current project) |
| Cursor | Add `{ "flowleap": { "command": "flowleap", "args": ["mcp"] } }` under `mcpServers` in `~/.cursor/mcp.json` | `flowleap skills install --target cursor` (current project) |
| Gemini CLI | `gemini mcp add --scope user flowleap flowleap mcp` | `flowleap skills install --target gemini` (current project) |

## Access & Pricing

The CLI itself is open source (MIT) and free to install. The patent-data commands call the FlowLeap backend, which requires:

- **A FlowLeap account with an active Basic plan.** A trial is available — see [flowleap.co/en/pricing](https://flowleap.co/en/pricing).
...
`doctor`, `health`, `auth`, and `keys test` remain available without an active subscription, so you can always diagnose the setup. `doctor` reports `ready: false` until the account has an active entitlement, including an active trial.
...
Verify the MCP server and live tool registry with:
...
```bash
# User-wide MCP server
claude mcp add --transport stdio --scope user flowleap -- flowleap mcp
...
```bash
# User-wide MCP server
codex mcp add flowleap -- flowleap mcp
...
```bash
# User-wide MCP server
gemini mcp add --scope user flowleap flowleap mcp
